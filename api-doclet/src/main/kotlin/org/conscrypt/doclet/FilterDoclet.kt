...
class FilterDoclet : Doclet {
  ...
  private fun generateIndex() {
    ...
    html {
        body(title = docTitle, stylesheet = relativePath(indexPath, CSS_FILENAME)) {
          div("index-container") {
            h1(docTitle, "index-title")
            compose { classIndex.generateHtml() }
          }
        }
      }
      .let { Files.newBufferedWriter(indexPath).use { writer -> writer.write(it) } }
  }
  ...
  private fun generateClassFile(classInfo: ClassInfo) {
    ...
    html {
        body(
          ...
          compose { classInfo.generateHtml() }
        }
      }
      .let { Files.newBufferedWriter(classFilePath).use { writer -> writer.write(it) } }
  }
  ...
  override fun getSupportedOptions(): Set<Doclet.Option> {
    return setOf<Doclet.Option>(
      StringOption("-d", "<directory>", "Destination directory for output files") { d: String ->
        outputDir = d
      },
      StringOption("-doctitle", "<title>", "Document title") { t: String -> docTitle = t },
      StringOption("-windowtitle", "<title>", "Window title") { w: String -> windowTitle = w },
      StringOption("-link", "<link>", "Link") { l: String -> baseUrl = l },
      BooleanOption("-notimestamp", "Something") { noTimestamp = true },
    )
  }
}
