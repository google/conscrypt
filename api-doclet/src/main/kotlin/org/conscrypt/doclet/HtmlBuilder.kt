...
class HtmlBuilder {
  ...
  fun body(title: String, stylesheet: String, content: Block) {
    text(
      """
             ...
             <body>"""
        .trimIndent() + content.render() + "</body></html>"
    )
  }
  ...
  fun <T> items(
    ...
    collection.forEach { li(cssClass = cssClass) { transform(it) } }
  }
  ...
  fun <T> row(
    ...
    tr(cssClass = rowClass) { transform(item) }
  }
  ...
  fun <T> rowGroup(
    ...
    if (rows.isNotEmpty()) {
      title?.let { tr { th(colspan = colspan) { strong(it) } } }
      rows.forEach { tr { transform(it) } }
    }
  }
}
...
fun exampleSubfunction() = html {
  ...
  listOf("one", "two", "three").forEach { h1(it) }
}

fun example() = html {
  val fruits = listOf("Apple", "Banana", "Cherry")
  body(stylesheet = "path/to/stylesheet.css", title = "Page Title") {
    div(cssClass = "example-class") {
      text { "This is a div" }
      h1 { text("Heading1a") }
      h2 { a("www.google.com", "Heading with a link") }
      h3("Heading with CSS class", "my-class")
      h2("h2", "my-class")
      p("Hello world")
      compose { exampleSubfunction() }
      br()
      a("www.google.com") {
        text("a link with ")
        b("bold")
    ...
    ul {
      ...
      items(fruits)
      // Text transform
      items(fruits) { text("I like ${it}.") }
      // HTML transform with a CSS class
      items(fruits, "myclass") { a("www.google.com") { b(it) } }
    }
    ol("ol-class") { items((1..5).asIterable()) { text("Item $it") } }
  }
  val data1 = listOf(1, 2)
  val data2 = "3" to "4"
  val data3 = listOf("tag1" to "Some value", "tag2" to "Next Value", "tag3" to "Another value")

  table("table-class") {
    tr {
      th { text("First column") }
      th { text("Second column") }
    }
    tr("tr-class") {
      td("td-class") { text("Data 1") }
      td(colspan = 2, id = "foo") { text("Data 2") }
    }
    tr { td() { text("Data 3") } }
    row(data1, "c1") { a(href = "www.google.com") { text("$it") } }
