...
public class EdDsaTest {
  ...
  public void signAndVerifyWithOffset_works() throws Exception {
    ...
    assertArrayEquals(
      sig, Arrays.copyOfRange(outputBuffer, offset, offset + sig.length));
  ...
  public void convertPrivateKeyToAndFromKeySpec_works() throws Exception {
    byte[] rawPrivateKey = decodeHex("9d61b19deffd5a60ba844af492ec2cc44449c5697b326919703bac031cae7f60");
    KeyFactory keyFactory = KeyFactory.getInstance("Ed25519", conscryptProvider);
    PrivateKey privateKey =
        keyFactory.generatePrivate(new RawKeySpec(rawPrivateKey));
    ...
    assertArrayEquals(
        expectedPkcs8Prefix, Arrays.copyOfRange(encodedPrivateKey, 0, expectedPkcs8Prefix.length));
    assertArrayEquals(
        rawPrivateKey, Arrays.copyOfRange(encodedPrivateKey, expectedPkcs8Prefix.length, encodedPrivateKey.length));

    // PKCS8EncodedKeySpec returns the same encoding as getEncoded().
    PKCS8EncodedKeySpec privateKeySpec = keyFactory.getKeySpec(privateKey, PKCS8EncodedKeySpec.class);
  ...
  public void convertPublicKeyToFromRawKeySpec_works() throws Exception {
    byte[] rawPublicKey = decodeHex("d75a980182b10ab7d54bfed3c964073a0ee172f3daa62325af021a68f707511a");
    KeyFactory keyFactory = KeyFactory.getInstance("Ed25519", conscryptProvider);
    PublicKey publicKey =
        keyFactory.generatePublic(new RawKeySpec(rawPublicKey));
    ...
    assertArrayEquals(expectedX509Prefix, Arrays.copyOfRange(encodedPublicKey, 0, expectedX509Prefix.length));
    assertArrayEquals(
        rawPublicKey,
        Arrays.copyOfRange(encodedPublicKey, expectedX509Prefix.length, encodedPublicKey.length));
