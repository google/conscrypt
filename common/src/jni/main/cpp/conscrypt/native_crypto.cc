...
static void NativeCrypto_SLHDSA_SHA2_128S_generate_key(
    JNIEnv* env, jclass, jbyteArray outPublicArray,
    jbyteArray outPrivateArray) {
  CHECK_ERROR_QUEUE_ON_RETURN;
  JNI_TRACE("SLHDSA_SHA2_128S_generate_key(%p, %p)", outPublicArray,
            outPrivateArray);
  ...
  JNI_TRACE("SLHDSA_SHA2_128S_generate_key(%p, %p) => success", outPublicArray,
            outPrivateArray);
}

static jbyteArray NativeCrypto_SLHDSA_SHA2_128S_sign(JNIEnv* env, jclass,
                                                     jbyteArray data,
  ...
  if (!SLHDSA_SHA2_128S_sign(
          result, reinterpret_cast<const unsigned char*>(privateKeyArray.get()),
    ...
    conscrypt::jniutil::throwExceptionFromBoringSSLError(
        env, "SLHDSA_SHA2_128S_sign");
    return nullptr;
  }

  ScopedLocalRef<jbyteArray> resultRef(
      env,
      env->NewByteArray(static_cast<jsize>(SLHDSA_SHA2_128S_SIGNATURE_BYTES)));
  if (resultRef.get() == nullptr) {
    return nullptr;
  }
...
static jint NativeCrypto_SLHDSA_SHA2_128S_verify(JNIEnv* env, jclass,
                                                 jbyteArray data,
                                                 jbyteArray sig,
                                                 jbyteArray publicKey) {
  CHECK_ERROR_QUEUE_ON_RETURN;
  ...
  JNI_TRACE("NativeCrypto_SLHDSA_SHA2_128S_verify(%p, %p, %p) => %d", publicKey,
            sig, data, result);
  return static_cast<jint>(result);
}
