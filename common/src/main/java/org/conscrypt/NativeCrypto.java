...
public final class NativeCrypto {
    ...
    static native byte[] wrap_RSA_private_key_pkcs8(byte[] rawKeyBytes);
    static native byte[] wrap_RSA_public_key_x509(byte[] rawKeyBytes);

    static native byte[] unwrap_RSA_private_key_pkcs8(byte[] rawKeyBytes);
    static native byte[] unwrap_RSA_public_key_x509(byte[] rawKeyBytes);
    ...
    static native byte[] wrap_EC_private_key_pkcs8(byte[] rawKeyBytes);

    static native byte[] wrap_EC_public_key_x509(byte[] rawKeyBytes, String curveName);

    static native byte[] unwrap_EC_private_key_pkcs8(byte[] rawKeyBytes);

    static native byte[] unwrap_EC_public_key_x509(byte[] rawKeyBytes);
    ...
    public static native byte[] XWING_public_key_from_seed(byte[] privateKeySeed);
    ...
    static native void EVP_PKEY_CTX_set1_signature_context_string(long ctx, byte[] contextString);
    ...
    /**
     * Returns an ASN.1 OID in dotted-decimal notation (eg, "1.3.14.3.2.26" for SHA-1) from the
     * given reference even if it's not registered in BoringSSL.
     */
    static native String asn1_read_oid_raw(long cbsRef) throws IOException;

    /**
     * Parses an ASN.1 BIT STRING and returns its payload checking that the unused bits
     * padding byte matches the expected value. Used for Composite ML-DSA keys.
     */
    static native byte[] asn1_read_bitstring_payload(long cbsRef, int expectedUnusedBits)
            throws IOException;
    ...
    /**
     * Writes the given data into the given reference as an ASN.1-encoded bit string with 0 padding.
     */
    static native void asn1_write_bitstring(long cbbRef, byte[] data) throws IOException;
    ...
    /**
     * Writes the given OID (which must be in dotted-decimal notation) into the given reference
     * even if it's not registered in BoringSSL.
     */
    static native void asn1_write_oid_raw(long cbbRef, String oid) throws IOException;
